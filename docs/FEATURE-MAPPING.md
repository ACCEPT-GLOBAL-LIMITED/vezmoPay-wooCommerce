...
| Express wallets (Apple Pay / Google Pay) | Not shown in the embedded payment form. | Hosted pages have them. See the **Wallet buttons in the embed iframe** row in `VEZMOPAY-API-CONTRACT.md`, which is the single statement of this — the plugin already sends the authorize-mode handshake a future build would need. |
