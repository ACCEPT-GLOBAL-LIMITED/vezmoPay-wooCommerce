...
| Wallet buttons in the embed iframe | **The canonical statement — every other document defers to this row.** The embedded Payment Element draws no Apple Pay / Google Pay buttons on the current build; the hosted vezmo-user pages do. The plugin already asks for the handshake a future build would need — `wallets=0&walletMode=authorize` on the frame URL in iframe mode, `wallets: 'authorize'` through the SDK in element mode — so that a wallet approval is held until WooCommerce has created an order rather than charging before one exists (see `assets/js/checkout-inline.js`). Until a build honours it, wallets in the payment box are **not available**, and `docs/TEST-CHECKLIST.md` §22 is pending that build. |
